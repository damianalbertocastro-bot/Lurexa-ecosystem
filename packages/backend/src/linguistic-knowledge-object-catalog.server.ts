...
/**
 * Linguistic-semantic extension for the Dominican-Spanish-to-English transfer
 * corpus. Objects already owned by the curriculum catalog are referenced by ID
 * in the pattern map rather than duplicated here.
 */
const linguisticObjects: readonly KnowledgeObjectV1[] = [
...
const byId = new Map(linguisticObjects.map((entry) => [entry.id, entry]));
...
export function getLinguisticKnowledgeObjectById(id: string): KnowledgeObjectV1 | null {
  return byId.get(id) ?? null;
}

export function listLinguisticKnowledgeObjects(): KnowledgeObjectV1[] {
  return [...linguisticObjects];
}
