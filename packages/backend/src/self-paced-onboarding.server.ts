...
function structuredA1Lesson(spec: A1LessonBlueprint): Lesson {
  ...
  return {
    ...
    contentBlocks: [
      ...
      { id: `${prefix}-speak`, type: "interactive", order: 5, data: { capability: { schemaVersion: "1", id: `${prefix}-recorded-speaking`, kind: "recorded_speaking", stage: "PHONETICS_FOCUS", title: "Say it clearly", instructions: "Record one short, meaningful response. Focus on understandable words, stress, and phrase endings—not accent erasure.", competencyIds: [...spec.competencies, "EN.A1.PHON.INTELLIGIBLE_CORE_PHRASES"], estimatedMinutes: 3, required: true, prompt: spec.spokenPrompt, locale: "en-US", minimumSeconds: 3, maximumSeconds: 45, evidencePurpose: "performance" } } },
