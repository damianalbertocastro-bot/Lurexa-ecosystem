...
/**
 * Lurexa Mind's initial deterministic learning-intelligence implementation.
 *
 * This module is deliberately storage-agnostic and authorization-agnostic:
 * Core supplies the already-authorized evidence projection and Core alone may
 * approve/persist the resulting candidate observations.
 */
export class ConservativeLearningIntelligenceService {
  ...
  async interpretLearnerEvidence(request: LearnerInterpretationRequest): Promise<LearnerInterpretationResult> {
    ...
    if (includeAllDomains || requestedDomains.has("goal")) {
      ...
      if (latestGoalEvidence && goal) {
        insights.push({
          ...
          domain: "goal",
          summary: `Learner goal: ${goal}`,
          confidence: 1,
          basedOnEvidenceIds: [latestGoalEvidence.id],
          data: { kind: "goals", goals: [goal] },
          generatedAt: now,
        });
      }
    }
    ...
    return {
      learnerId: request.learnerId,
      insights,
      evidenceIds: [...new Set(insights.flatMap((insight) => insight.basedOnEvidenceIds))],
      generatedAt: now,
    };
  }
}
