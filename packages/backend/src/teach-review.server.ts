...
function requireReviewerRole(role: unknown): asserts role is TrustedTeachReviewerRole {
  if (role !== "admin" && role !== "super_admin") throw new Error("Trusted Teach reviewer access is required.");
}

async function loadCredentialDefinitions(): Promise<TeachCredentialDefinition[]> {
  const snapshot = await getServerFirestore().collection("teachCredentialDefinitions").where("active", "==", true).get();
...
function matchingEvidenceIds(definition: TeachCredentialDefinition, evidence: TeachEvidenceSubmission[]): string[] {
  const competencyIds = definition.requirements.filter((requirement) => requirement.type === "verified-evidence" && requirement.competencyId).map((requirement) => requirement.competencyId!);
  return evidence.filter((item) => item.status === "verified").filter((item) => competencyIds.length === 0 || item.competencyIds.some((id) => competencyIds.includes(id))).map((item) => item.id);
}

function verificationCode(userId: string, credentialId: string): string {
  const digest = createHash("sha256").update(`${userId}:${credentialId}`).digest("hex").slice(0, 16).toUpperCase();
  return `LTX-${digest.slice(0, 4)}-${digest.slice(4, 8)}-${digest.slice(8, 12)}-${digest.slice(12, 16)}`;
}

function credentialRecommendation(userId: string, awards: TeachCredentialAward[], sourceEvidenceIds: string[]): TeachRecommendation {
  const award = awards[0]!;
  return { id:`teach-mvp-${userId}`, userId, title:awards.length===1?"You earned a new professional credential.":`You earned ${awards.length} new professional credentials.`, rationale:`Verified professional evidence now satisfies the requirements for ${award.credentialId}${awards.length>1?" and additional credentials":""}. Your next step should build on that demonstrated capability.`, actionLabel:"View credentials", actionHref:"/certifications", priority:"high", status:"active", sourceEvidenceIds, createdAt:new Date().toISOString() };
}
function rejectedEvidenceRecommendation(userId: string, evidence: TeachEvidenceSubmission): TeachRecommendation {
  return { id:`teach-mvp-${userId}`, userId, title:"Revise your professional evidence before resubmitting.", rationale:evidence.reviewerNote||"The submitted evidence needs another iteration before it can be verified as professional evidence.", actionLabel:"Review evidence portfolio", actionHref:"/growth", priority:"high", status:"active", sourceEvidenceIds:[evidence.id], createdAt:new Date().toISOString() };
}
...
export const TeachReviewServerService = {
  ...
  async listSubmittedEvidence(actor: TrustedTeachReviewer): Promise<TeachEvidenceSubmission[]> {
    ...
    const snapshot = await getServerFirestore().collection("teachEvidence").where("status", "==", "submitted").orderBy("createdAt", "asc").get();
  ...
  async reconcileEducatorState(userId: string, actorId = "system:teach-credential-engine") {
    ...
    const publicRecords: TeachPublicCredentialRecord[] = [];

    for (const definition of definitions) {
      if (existingCredentialIds.has(definition.id)) continue;
      if (!evaluateTeachCredential(definition, profile, enrollments, evidence).eligible) continue;
      const id = `${userId}_${definition.id}`;
      const code = verificationCode(userId, definition.id);
      newlyAwardedCredentials.push({ id, credentialId:definition.id, userId, awardedAt:timestamp, awardedBy:actorId, evidenceIds:matchingEvidenceIds(definition,evidence), verificationCode:code });
      publicRecords.push({ verificationCode:code, credentialId:definition.id, credentialName:definition.name, credentialDescription:definition.description, educatorDisplayName:profile?.displayName ?? "Lurexa educator", issuer:"Lurexa Learning Technologies", awardedAt:timestamp, status:"valid" });
    }

    let recommendation: TeachRecommendation | null = null;
    if (profile) recommendation = newlyAwardedCredentials.length > 0 ? credentialRecommendation(userId,newlyAwardedCredentials,Array.from(new Set(newlyAwardedCredentials.flatMap((award)=>award.evidenceIds)))) : TeachMindService.recommendNextStep(profile,enrollments,evidence);

    const batch = database.batch();
    for (const award of newlyAwardedCredentials) batch.set(database.collection("teachCredentialAwards").doc(award.id), award, { merge:false });
    for (const record of publicRecords) batch.set(database.collection("teachPublicCredentials").doc(record.verificationCode), record, { merge:false });
    if (recommendation) batch.set(database.collection("teachRecommendations").doc(recommendation.id), recommendation, { merge:false });
    if (newlyAwardedCredentials.length > 0 || recommendation) await batch.commit();
    return { newlyAwardedCredentials, recommendation };
  },

  async getPublicCredential(code: string): Promise<TeachPublicCredentialRecord | null> {
    const snapshot = await getServerFirestore().collection("teachPublicCredentials").doc(code.trim().toUpperCase()).get();
    return snapshot.exists ? (snapshot.data() as TeachPublicCredentialRecord) : null;
  },

  async reviewEvidence(actor: TrustedTeachReviewer, evidenceId: string, decision: TeachEvidenceReviewDecision, reviewerNote: string): Promise<TeachEvidenceReviewResult> {
    requireReviewerRole(actor.role);
    if (!reviewerNote.trim()) throw new Error("A reviewer note is required.");
    const database = getServerFirestore();
    ...
    const reviewedEvidence = await database.runTransaction(async (transaction) => {
      ...
      const next: TeachEvidenceSubmission = { ...evidence, status:decision, reviewerId:actor.uid, reviewerNote:reviewerNote.trim(), reviewedAt:timestamp, updatedAt:timestamp, ...(decision==="verified"?{verifiedAt:timestamp}:{}) };
