...
export function ProductBridge({ bridge, title = "Continue with your context", description, className = "", onContinue }: ProductBridgeProps) {
  return (
    <section className={`overflow-hidden rounded-[28px] border border-indigo-100 bg-[var(--lx-surface)] shadow-[0_16px_40px_rgba(49,95,215,.09)] ${className}`} aria-labelledby={`${bridge.bridgeId}-title`}>
      <div className="bg-[linear-gradient(120deg,rgba(79,70,229,.10),rgba(56,189,248,.10),rgba(45,212,191,.10))] p-5 sm:p-6">
        ...
        <p className="mt-1 max-w-2xl text-sm leading-6 text-[var(--lx-muted)]">{description ?? `Move from Lurexa ${bridge.source} to Lurexa ${bridge.destination} without starting over.`}</p>
        ...
        <div className="mt-6 flex items-center gap-3" aria-label={`Bridge from ${bridge.source} to ${bridge.destination}`}>
          <div className="min-w-0 flex-1 rounded-2xl border border-white/80 bg-white/90 px-4 py-3 shadow-sm">
            <span className="text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--lx-muted)]">From</span>
            <div className="mt-0.5 truncate text-sm font-bold capitalize text-[var(--lx-ink)]">Lurexa {bridge.source}</div>
          </div>
          ...
          <div className="min-w-0 flex-1 rounded-2xl border border-white/80 bg-white/90 px-4 py-3 shadow-sm">
            <span className="text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--lx-muted)]">To</span>
            <div className="mt-0.5 truncate text-sm font-bold capitalize text-[var(--lx-ink)]">Lurexa {bridge.destination}</div>
          </div>
        </div>

        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          <p className="text-xs leading-5 text-[var(--lx-muted)]">Purpose-scoped · server-validated · expires automatically</p>
