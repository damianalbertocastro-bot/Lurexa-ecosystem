...
export const Card: React.FC<CardProps> = ({ children, title, subtitle, action, className = "" }) => (
  <div className={`animate-fade-slide-up rounded-[22px] border border-[var(--lx-border)] bg-[var(--lx-surface)] p-6 shadow-[var(--lx-card-shadow)] transition duration-200 hover:shadow-[var(--lx-card-hover-shadow)] ${className}`}>
