...
const nodeStyle: Record<AdaptivePathNodeView["state"], string> = {
  ...
  locked: "border-[var(--lx-border)] bg-[var(--lx-canvas)] opacity-70",
...
export function AdaptiveLearningPath({ path, className = "", onSelectNode }: AdaptiveLearningPathProps) {
  return (
    <section className={`rounded-[28px] border border-[var(--lx-border)] bg-[var(--lx-surface)] p-5 shadow-[0_16px_40px_rgba(15,23,42,.07)] sm:p-6 ${className}`} aria-labelledby="adaptive-path-title">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-[var(--lx-muted)]">Lurexa can add targeted support around your path without silently rewriting required learning.</p>
        </div>
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--lx-muted)]">{index + 1}</span>
                  <span className="rounded-full bg-white/80 px-2.5 py-1 text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--lx-muted)]">{reasonLabel[node.reason]}</span>
                  <span className="rounded-full bg-white/80 px-2.5 py-1 text-[11px] font-semibold capitalize text-[var(--lx-muted)]">{node.product}</span>
                </div>
                {node.required ? <span className="text-xs font-semibold text-[var(--lx-muted)]">Required</span> : <span className="text-xs font-semibold text-sky-700">Adaptive overlay</span>}
              </div>
              <div className="mt-2 text-base font-bold tracking-[-.02em] text-[var(--lx-ink)]">{node.title}</div>
              {node.knowledgeObjectIds.length > 0 && <p className="mt-1 text-xs text-[var(--lx-muted)]">Targets {node.knowledgeObjectIds.length} mapped knowledge object{node.knowledgeObjectIds.length === 1 ? "" : "s"}</p>}
            </button>
          </li>
        ))}
      </ol>

      {path.nodes.length === 0 && <div className="mt-6 rounded-2xl border border-dashed border-[var(--lx-border)] bg-[var(--lx-canvas)] p-5 text-sm text-[var(--lx-muted)]">No route is available yet. Lurexa needs current curriculum context before it can build this view.</div>}
    </section>
  );
}
