...
const productLabelClass: Record<LurexaProduct, string> = {
  learn: "text-[#592bd6]",
  coach: "text-[#0ba5a8]",
  teach: "text-[var(--lx-secondary)]",
  admin: "text-[var(--color-brand-navy)]",
  insight: "text-[#2160df]",
  studio: "text-[#8b3bd9]",
  campus: "text-[var(--color-brand-navy)]",
};
...
function ProductGlyph({ product }: { product: LurexaProduct }) {
  ...
  if (product === "coach") {
    return <svg viewBox="0 0 80 80" className="h-full w-full" style={{ display: "block", width: "100%", height: "100%" }} aria-hidden="true">
      ...
      <path d="M17 40h7l4-10 6 20 6-17 5 12 4-5h11" fill="none" stroke="var(--lx-accent)" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" />
      <circle cx="58" cy="20" r="10" fill="var(--color-brand-navy)" />
  ...
  if (product === "teach") {
    return <svg viewBox="0 0 64 64" className="h-full w-full" style={{ display: "block", width: "100%", height: "100%" }} aria-hidden="true">
      <path d="M7 23h50l-9-11H16L7 23Z" fill="var(--color-brand-navy)" />
      <path d="M15 25h34v7H15z" fill="var(--color-brand-navy)" />
      <path d="M30 32v15" stroke="var(--color-brand-navy)" strokeWidth="4" />
      <circle cx="30" cy="51" r="3" fill="var(--lx-accent)" />
    </svg>;
  }

  if (product === "admin") {
    return <svg viewBox="0 0 80 80" className="h-full w-full" style={{ display: "block", width: "100%", height: "100%" }} aria-hidden="true">
      <path d="M40 6 68 16v22c0 18-11 29-28 36C23 67 12 56 12 38V16L40 6Z" fill="var(--color-brand-navy)" />
      ...
      <circle cx="59" cy="55" r="10" fill="var(--lx-accent)" />
      <path d="m54 55 4 4 7-9" fill="none" stroke="var(--color-brand-navy)" strokeWidth="3.5" strokeLinecap="round" />
    </svg>;
  }

  if (product === "insight") {
    return <svg viewBox="0 0 80 80" className="h-full w-full" style={{ display: "block", width: "100%", height: "100%" }} aria-hidden="true">
      <circle cx="34" cy="34" r="23" fill="var(--color-brand-navy)" />
      ...
      <circle cx="46" cy="24" r="4" fill="var(--lx-accent)" />
  ...
  if (product === "campus") {
    return <svg viewBox="0 0 80 80" className="h-full w-full" style={{ display: "block", width: "100%", height: "100%" }} aria-hidden="true">
      <path d="M40 8 10 26h60L40 8Z" fill="var(--color-brand-navy)" />
      <rect x="10" y="27" width="60" height="5" rx="2.5" fill="var(--color-brand-navy)" />
      <rect x="16" y="36" width="11" height="24" rx="4" fill="#2160DF" />
      <rect x="34.5" y="36" width="11" height="24" rx="4" fill="var(--lx-accent)" />
      <rect x="53" y="36" width="11" height="24" rx="4" fill="#2160DF" />
      <rect x="10" y="64" width="60" height="8" rx="3" fill="var(--color-brand-navy)" />
    </svg>;
  }
  ...
  return <svg viewBox="0 0 80 80" className="h-full w-full" style={{ display: "block", width: "100%", height: "100%" }} aria-hidden="true">
    ...
    <rect x="8" y="48" width="28" height="24" rx="7" fill="var(--color-brand-navy)" />
    <path d="M52 49h12v7h8v12h-8v7H52v-7h-8V56h8v-7Z" fill="var(--lx-accent)" />
...
export function ProductMark({ product = "learn", inverse = false, compact = false, size = "md", className = "", ...props }: ProductMarkProps) {
  const wordmark = inverse ? "text-white" : "text-[var(--color-brand-navy)]";
