...
export function MemoryThread({ thread, className = "" }: MemoryThreadProps) {
  return (
    <section className={`rounded-[28px] border border-[var(--lx-border)] bg-[var(--lx-surface)] p-5 shadow-[0_16px_40px_rgba(15,23,42,.07)] sm:p-6 ${className}`} aria-labelledby="memory-thread-title">
      ...
      {thread.currentSummary && <p className="mt-1 max-w-2xl text-sm leading-6 text-[var(--lx-muted)]">{thread.currentSummary}</p>}

      <ol className="relative mt-6 space-y-5 before:absolute before:bottom-3 before:left-[7px] before:top-3 before:w-px before:bg-[var(--lx-canvas)]">
        {thread.events.map((event) => (
          <li key={event.id} className="relative pl-8">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--lx-muted)]">
              ...
              <span className="rounded-full bg-[var(--lx-canvas)] px-2 py-0.5 font-semibold text-[var(--lx-muted)]">{eventLabel[event.kind]}</span>
            </div>
            <h3 className="mt-1 text-sm font-bold text-[var(--lx-ink)]">{event.title}</h3>
            <p className="mt-1 text-sm leading-6 text-[var(--lx-muted)]">{event.summary}</p>
          </li>
        ))}
      </ol>

      {thread.events.length === 0 && <div className="mt-5 rounded-2xl border border-dashed border-[var(--lx-border)] bg-[var(--lx-canvas)] p-5 text-sm text-[var(--lx-muted)]">This thread is intentionally empty until governed learning evidence exists.</div>}

      <p className="mt-5 text-xs leading-5 text-[var(--lx-muted)]">This is a learning-development projection, not a raw activity log or a complete record of everything you have done.</p>
    </section>
  );
}
