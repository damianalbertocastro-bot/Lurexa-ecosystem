...
export default function AssessmentPage() {
  ...
  return <TeachShell active="Assessment"><TeachPrivate><main className="mx-auto max-w-[1180px] px-5 py-10 sm:px-8">
    ...
    <section className="mt-7 grid gap-5 lg:grid-cols-[.75fr_1.25fr]">
      <article className="rounded-[28px] border border-[var(--lx-surface)] bg-white p-7"><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">REQUEST ASSESSMENT</p><h2 className="mt-2 text-2xl font-black">Choose what needs verification.</h2><div className="mt-5 flex flex-wrap gap-2">{(["cefr","teaching-competency"] as TeachAssessmentDomain[]).map((domain)=><Button key={domain} type="button" onClick={()=>toggleDomain(domain)} className={`min-h-11 rounded-full border px-4 text-sm font-extrabold ${domains.includes(domain)?"border-[var(--lx-primary)] bg-[var(--lx-primary)] text-white":"border-[var(--lx-border)] bg-white text-[var(--lx-muted)]"}`}>{domain==="cefr"?"English CEFR":"Teaching competency"}</Button>)}</div><label className="mt-5 block text-xs font-extrabold text-[var(--lx-muted)]">COMPETENCY IDS<Input value={competencies} onChange={(event)=>setCompetencies(event.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-[var(--lx-border)] px-3 text-sm font-medium" placeholder="speaking-instruction, assessment-literacy" /></label><label className="mt-5 block text-xs font-extrabold text-[var(--lx-muted)]">CONTEXT FOR ASSESSOR<textarea value={note} onChange={(event)=>setNote(event.target.value)} rows={4} className="mt-2 w-full rounded-xl border border-[var(--lx-border)] p-3 text-sm font-medium" placeholder="What professional capability are you ready to demonstrate?" /></label><Button type="button" onClick={()=>void submit()} disabled={saving||domains.length===0} className="mt-5 min-h-12 rounded-xl bg-gradient-to-br from-[var(--lx-primary)] to-[var(--lx-secondary)] px-5 text-sm font-extrabold text-white disabled:opacity-50">{saving?"Submitting…":"Request trusted assessment"}</Button></article>

      <article className="rounded-[28px] border border-[var(--lx-surface)] bg-white p-7"><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">ASSESSMENT HISTORY</p><h2 className="mt-2 text-2xl font-black">Your verification pipeline</h2>{assessments.length===0?<p className="mt-5 rounded-2xl bg-[var(--lx-surface)] p-5 text-sm text-[var(--lx-muted)]">No trusted assessments requested yet.</p>:assessments.map((assessment)=><div key={assessment.id} className="mt-5 border-t border-[var(--lx-surface)] pt-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><b className="capitalize">{assessment.domains.join(" + ").replaceAll("-"," ")}</b><p className="mt-1 text-xs text-[var(--lx-muted)]">Requested {new Date(assessment.requestedAt).toLocaleDateString()}</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-extrabold capitalize ${assessment.status==="completed"?"bg-[var(--lx-surface)] text-[var(--lx-success)]":"bg-[var(--lx-surface)] text-[var(--lx-primary)]"}`}>{assessment.status.replaceAll("_"," ")}</span></div></div>)}</article>
    </section>
