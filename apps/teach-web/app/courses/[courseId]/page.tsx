...
export default function CourseRuntimePage() {
  ...
  return <TeachShell active="Learn"><TeachPrivate><main className="mx-auto max-w-[1180px] px-5 py-10 sm:px-8">{loading ? <div role="status" className="py-20 text-center text-sm font-bold text-[var(--lx-muted)]">Loading course…</div> : !course ? <div className="rounded-[28px] border border-[var(--lx-border)] bg-[var(--lx-surface)] p-8"><h1 className="text-2xl font-black">Course not found</h1><a href="/courses" className="mt-4 inline-flex text-sm font-extrabold text-[var(--lx-secondary)]">Back to professional learning →</a></div> : <><section className="rounded-[34px] bg-gradient-to-br from-[var(--color-brand-navy)] via-[var(--color-brand-navy)] to-[var(--lx-primary)] p-7 text-white sm:p-10"><p className="text-[10px] font-extrabold uppercase tracking-[.18em] text-[var(--lx-accent)]">{course.track.replaceAll("-", " ")}</p><h1 className="mt-3 max-w-4xl text-4xl font-black tracking-[-.055em] sm:text-6xl">{course.title}</h1><p className="mt-5 max-w-3xl text-base leading-7 text-indigo-100">{course.description}</p><div className="mt-8"><div className="h-3 overflow-hidden rounded-full bg-white/15"><div className="h-full rounded-full bg-[var(--lx-accent)]" style={{width:`${enrollment?.progressPercent ?? 0}%`}}/></div><p className="mt-2 text-sm font-extrabold text-indigo-100">{enrollment?.progressPercent ?? 0}% complete</p></div></section>{error && <p role="alert" className="mt-5 rounded-2xl bg-[var(--lx-destructive)] p-4 text-sm font-bold text-[var(--lx-destructive)]">{error}</p>}<section className="mt-7 grid gap-5 lg:grid-cols-[1fr_.36fr]"><article className="rounded-[28px] border border-[var(--lx-border)] bg-[var(--lx-surface)] p-7"><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">LEARNING SEQUENCE</p><h2 className="mt-2 text-2xl font-black">Modules and evidence</h2>{course.modules.sort((a,b)=>a.order-b.order).map((module)=><div key={module.id} className="mt-6 border-t border-[var(--lx-border)] pt-6"><div className="flex items-start gap-4"><Button type="button" onClick={()=>toggleModule(module.id)} disabled={saving===module.id} aria-pressed={completed.has(module.id)} className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl border text-sm font-black ${completed.has(module.id)?"border-[var(--lx-success)] bg-[var(--lx-surface)] text-[var(--lx-success)]":"border-[var(--lx-border)] bg-[var(--lx-surface)] text-[var(--lx-primary)]"}`}>{saving===module.id?"…":completed.has(module.id)?"✓":module.order}</Button><div className="min-w-0"><h3 className="text-lg font-black">{module.title}</h3><p className="mt-2 text-sm leading-6 text-[var(--lx-muted)]">{module.description || "Learn the concept, apply it to educator practice, and record meaningful evidence when required."}</p>{module.evidenceRequired && <a href={`/growth?courseId=${course.id}&moduleId=${module.id}`} className="mt-3 inline-flex min-h-11 items-center text-sm font-extrabold text-[var(--lx-secondary)]">Submit evidence for this module →</a>}</div></div></div>)}</article><aside className="space-y-5"><article className="rounded-[28px] border border-[var(--lx-border)] bg-[var(--lx-surface)] p-6"><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">CAPABILITIES</p><div className="mt-4 flex flex-wrap gap-2">{course.competencyIds.map((item)=><span key={item} className="rounded-full bg-[var(--lx-surface)] px-3 py-2 text-xs font-extrabold text-[var(--lx-primary)]">{item.replaceAll("-", " ")}</span>)}</div></article><article className="rounded-[28px] bg-[var(--lx-warning)] p-6"><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-warning)]">COMPLETION RULE</p><p className="mt-3 text-sm leading-6 text-[var(--lx-muted)]">Module completion tracks participation. Evidence-required modules still need appropriate portfolio evidence before they can support a credential.</p></article></aside></section></>}</main></TeachPrivate></TeachShell>;
}
