...
export function TeachShell({ active, children }: { active: string; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#f5f7ff] text-[#0b1f5f]">
    ...
    {children}
    <footer className="border-t border-[#dfe6f8] bg-white"><div className="mx-auto grid max-w-[1440px] gap-7 px-5 py-10 sm:px-8 md:grid-cols-[1fr_auto]"><div><ProductMark product="teach"/><p className="mt-4 max-w-xl text-sm leading-6 text-[#6677a5]">Grow your language, teaching practice, professional evidence, and professional network in one connected educator experience.</p></div><div className="flex flex-wrap gap-x-5 gap-y-3 text-sm font-bold text-[#53679f]"><Link href="/courses">Learning</Link><Link href="/community">Community</Link><Link href="/growth">Growth</Link><Link href="/assessment">Assessment</Link><Link href="/certifications">Credentials</Link><a href={ecosystemUrl}>Lurexa ecosystem ↗</a></div></div></footer>
  </div>;
}
