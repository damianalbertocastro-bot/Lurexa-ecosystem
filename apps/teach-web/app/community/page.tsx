...
export default function CommunityPage(){
  ...
  return <TeachShell active="Community"><TeachPrivate><main className="mx-auto max-w-[1440px] px-5 py-10 sm:px-8"><section className="rounded-[34px] bg-gradient-to-br from-[#fff8ed] to-[#f7f3ff] p-7 sm:p-10"><div className="grid gap-8 lg:grid-cols-[1fr_.7fr] lg:items-end"><div><p className="text-[11px] font-extrabold tracking-[.18em] text-[#a05e20]">LUREXA TEACH COMMUNITY</p><h1 className="mt-3 max-w-3xl text-4xl font-black tracking-[-.055em] sm:text-6xl">Professional growth should have people in it.</h1><p className="mt-5 max-w-2xl text-base leading-7 text-[#6c6372]">Ask, share, practice, reflect, mentor, and learn with educators who understand the work. Contributions are stored in your professional identity and may become evidence when explicitly eligible.</p></div><Button onClick={()=>setShowComposer((value)=>!value)} className="min-h-12 justify-self-start rounded-xl bg-[var(--color-brand-navy)] px-6 text-sm font-extrabold text-white lg:justify-self-end">{showComposer?"Close composer":"Create a post"}</Button></div></section>

{showComposer&&<form onSubmit={submit} className="mt-6 rounded-[28px] border border-[var(--lx-surface)] bg-white p-6 sm:p-7"><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">NEW PROFESSIONAL CONTRIBUTION</p><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="text-sm font-extrabold text-[#30457f]">Circle<select value={circleId} onChange={(e)=>setCircleId(e.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-[#d7e0f6] bg-white px-4"><option value="">General Teach community</option>{circles.map(([id,name])=><option key={id} value={id}>{name}</option>)}</select></label><label className="text-sm font-extrabold text-[#30457f]">Title<Input required value={title} onChange={(e)=>setTitle(e.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-[#d7e0f6] px-4" placeholder="What do you want other teachers to discuss?"/></label></div><label className="mt-4 block text-sm font-extrabold text-[#30457f]">Contribution<textarea required value={body} onChange={(e)=>setBody(e.target.value)} className="mt-2 min-h-36 w-full rounded-xl border border-[#d7e0f6] p-4" placeholder="Share the classroom context, what you tried, what happened, or what you want feedback on."/></label><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="text-xs font-bold text-[var(--lx-muted)]">Posts may be marked evidence-eligible, but community activity is never automatically verified evidence.</p><Button disabled={busy} className="min-h-11 rounded-xl bg-[var(--lx-primary)] px-5 text-sm font-extrabold text-white disabled:opacity-60">{busy?"Publishing…":"Publish contribution"}</Button></div></form>}
{error&&<p role="alert" className="mt-5 rounded-2xl bg-[#fff0f2] p-4 text-sm font-bold text-[#b52c49]">{error}</p>}

<section className="mt-10"><div><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">PROFESSIONAL CIRCLES</p><h2 className="mt-2 text-3xl font-black tracking-[-.045em]">Find the context that matches your work.</h2></div><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{circles.map(([id,name,copy],i)=><article key={id} className="rounded-[24px] border border-[var(--lx-surface)] bg-white p-6"><div className={`grid h-12 w-12 place-items-center rounded-2xl font-black ${i%2?"bg-[#e7f9f8] text-[#147c7e]":"bg-[#eee9ff] text-[var(--lx-primary)]"}`}>{name.charAt(0)}</div><h3 className="mt-5 text-lg font-black">{name}</h3><p className="mt-4 text-sm leading-6 text-[var(--lx-muted)]">{copy}</p><Button type="button" onClick={async()=>{setCircleId(id); try{setPosts(await TeachService.listCommunityPosts(id));}catch(err){setError(err instanceof Error?err.message:"Circle could not be loaded.");}}} className="mt-5 min-h-11 text-sm font-extrabold text-[var(--lx-secondary)]">Open circle →</Button></article>)}</div></section>

<section className="mt-12 grid gap-5 lg:grid-cols-[1.35fr_.65fr]"><article className="rounded-[28px] border border-[var(--lx-surface)] bg-white p-7"><div className="flex items-end justify-between gap-3"><div><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">COMMUNITY FEED</p><h2 className="mt-2 text-2xl font-black">Conversations worth joining</h2></div>{circleId&&<Button onClick={()=>{setCircleId("");void load();}} className="text-sm font-extrabold text-[var(--lx-secondary)]">Show all</Button>}</div>{posts.length?posts.map((post)=><article key={post.id} className="mt-6 border-t border-[#edf1fb] pt-6"><div className="flex gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-[#eef3ff] font-black text-[var(--lx-secondary)]">{post.authorName.charAt(0)}</span><div><b className="text-sm text-[#536ba5]">{post.authorName}</b><h3 className="mt-1 text-lg font-black leading-6">{post.title}</h3><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-[var(--lx-muted)]">{post.body}</p><p className="mt-2 text-xs font-bold text-[#8994b4]">{new Date(post.createdAt).toLocaleDateString()} {post.evidenceEligible?"· Evidence eligible":""}</p></div></div></article>):<p className="mt-6 rounded-2xl bg-[var(--lx-surface)] p-5 text-sm leading-6 text-[var(--lx-muted)]">No contributions here yet. Start the professional conversation.</p>}</article><aside className="space-y-5"><article className="rounded-[28px] bg-gradient-to-br from-[var(--color-brand-navy)] to-[var(--lx-secondary)] p-7 text-white"><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-accent)]">WEEKLY CHALLENGE</p><h2 className="mt-3 text-2xl font-black">Share one classroom move that increased student talk time.</h2><Button onClick={()=>setShowComposer(true)} className="mt-6 min-h-11 rounded-xl bg-white px-4 text-sm font-extrabold text-[#26358c]">Contribute →</Button></article><article className="rounded-[28px] border border-[var(--lx-surface)] bg-white p-7"><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">COMMUNITY STANDARD</p><h2 className="mt-3 text-xl font-black">Useful professional exchange over engagement metrics.</h2><p className="mt-3 text-sm leading-6 text-[var(--lx-muted)]">Teach prioritizes contextual questions, classroom evidence, reflection, resources, and peer feedback—not follower counts or viral ranking.</p></article></aside></section></main></TeachPrivate></TeachShell>;
}
