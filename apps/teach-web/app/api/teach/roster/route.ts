...
import { getTeachInstructionalRoster } from "@lurexa/backend/teach-roster.server";
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const roster = await getTeachInstructionalRoster(actor);
    return Response.json(roster, { headers: privateNoStoreHeaders });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to load the instructional roster.";
