...
export default function TeachReviewPage() {
  ...
  return <TeachShell active="Dashboard"><TeachPrivate><main className="mx-auto max-w-[1180px] px-5 py-10 sm:px-8">
    <section className="rounded-[32px] bg-gradient-to-br from-[var(--color-brand-navy)] via-[#18368d] to-[#4825a8] p-8 text-white sm:p-10">
    ...
    <div className="mt-8 flex items-end justify-between gap-4"><div><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">REVIEW QUEUE</p><h2 className="mt-2 text-3xl font-black tracking-[-.045em]">Submitted evidence</h2></div><Button type="button" onClick={() => void loadQueue()} className="min-h-11 rounded-xl border border-[#d7e0f6] bg-white px-4 text-sm font-extrabold text-[var(--lx-secondary)]">Refresh</Button></div>
    ...
    {loading ? <div className="mt-6 rounded-[26px] border border-[var(--lx-surface)] bg-white p-8 text-sm text-[var(--lx-muted)]">Loading trusted review queue…</div> : evidence.length === 0 ? <div className="mt-6 rounded-[26px] border border-dashed border-[#ccd7f2] bg-white p-10 text-center"><b className="text-xl">No evidence is waiting for review.</b><p className="mt-2 text-sm text-[var(--lx-muted)]">Verified and rejected items leave this queue automatically.</p></div> : <section className="mt-6 space-y-5">{evidence.map((item) => <article key={item.id} className="rounded-[28px] border border-[var(--lx-surface)] bg-white p-7 shadow-[0_12px_30px_rgba(32,52,128,.05)]">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[.15em] text-[var(--lx-muted)]">{item.type.replaceAll("-", " ")}</p><h3 className="mt-2 text-2xl font-black tracking-[-.04em]">{item.title}</h3><p className="mt-2 text-xs font-bold text-[#8994b4]">Educator {item.userId} · Submitted {new Date(item.createdAt).toLocaleDateString()}</p></div><span className="rounded-full bg-[#fff3dc] px-3 py-1.5 text-xs font-extrabold text-[#a66013]">Awaiting review</span></div>
      <p className="mt-5 text-sm leading-7 text-[var(--lx-muted)]">{item.description}</p>
      {item.competencyIds.length > 0 && <div className="mt-5 flex flex-wrap gap-2">{item.competencyIds.map((id) => <span key={id} className="rounded-full bg-[#f0ecff] px-3 py-1.5 text-xs font-extrabold text-[var(--lx-primary)]">{id}</span>)}</div>}
      ...
      <label className="mt-6 block"><span className="text-xs font-extrabold uppercase tracking-[.12em] text-[var(--lx-muted)]">Reviewer note</span><textarea value={notes[item.id] ?? ""} onChange={(event) => setNotes((current) => ({ ...current, [item.id]: event.target.value }))} rows={4} className="mt-2 w-full rounded-2xl border border-[#ccd7f2] bg-[#fbfcff] p-4 text-sm text-[var(--color-brand-navy)] outline-none focus:border-[var(--lx-primary)] focus:ring-4 focus:ring-[var(--lx-primary)]/10" placeholder="Explain why this evidence demonstrates the competency, or what must improve before resubmission." /></label>
