...
export default function TeachReviewPage() {
  ...
  return <TeachShell active="Dashboard"><TeachPrivate><main className="mx-auto max-w-[1180px] px-5 py-10 sm:px-8">
    ...
    <div className="mt-8 flex items-end justify-between gap-4"><div><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">REVIEW QUEUE</p><h2 className="mt-2 text-3xl font-black tracking-[-.045em]">Submitted evidence</h2></div><Button type="button" onClick={() => void loadQueue()} className="min-h-11 rounded-xl border border-[var(--lx-border)] bg-[var(--lx-surface)] px-4 text-sm font-extrabold text-[var(--lx-secondary)]">Refresh</Button></div>

    {loading ? <div className="mt-6 rounded-[26px] border border-[var(--lx-border)] bg-[var(--lx-surface)] p-8 text-sm text-[var(--lx-muted)]">Loading trusted review queue…</div> : evidence.length === 0 ? <div className="mt-6 rounded-[26px] border border-dashed border-[var(--lx-border)] bg-[var(--lx-surface)] p-10 text-center"><b className="text-xl">No evidence is waiting for review.</b><p className="mt-2 text-sm text-[var(--lx-muted)]">Verified and rejected items leave this queue automatically.</p></div> : <section className="mt-6 space-y-5">{evidence.map((item) => <article key={item.id} className="rounded-[28px] border border-[var(--lx-border)] bg-[var(--lx-surface)] p-7 shadow-[var(--lx-card-shadow)]">
