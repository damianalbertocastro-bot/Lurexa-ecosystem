...
export default async function DocumentationSectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  return <DocsShell active={definition.title}><main className="mx-auto max-w-[1480px] px-5 py-12 sm:px-8 sm:py-16">
    <section className="grid gap-8 lg:grid-cols-[1fr_.72fr] lg:items-end"><div><p className="text-[10px] font-extrabold tracking-[.18em] text-[var(--lx-primary)]">{definition.eyebrow}</p><h1 className="mt-3 text-5xl font-black tracking-[-.065em] sm:text-6xl">{definition.title}</h1><p className="mt-5 max-w-3xl text-base leading-8 text-[#5d6f9d]">{definition.description}</p></div><aside className="rounded-[24px] border border-[var(--lx-surface)] bg-white p-6 shadow-[0_12px_30px_rgba(32,52,128,.055)]"><p className="text-[10px] font-extrabold tracking-[.16em] text-[var(--lx-muted)]">WHEN TO USE THIS DOMAIN</p><p className="mt-3 text-sm leading-7 text-[#53679f]">{definition.responsibility}</p></aside></section>

    <section className="mt-12 grid gap-5 lg:grid-cols-[.72fr_1.28fr]"><aside className="h-fit rounded-[28px] bg-gradient-to-br from-[var(--color-brand-navy)] via-[#19388f] to-[#4b28ae] p-7 text-white lg:sticky lg:top-28"><p className="text-[10px] font-extrabold tracking-[.18em] text-[var(--lx-accent)]">CANONICAL CONTENT</p><h2 className="mt-3 text-2xl font-black tracking-[-.04em]">These pages read directly from the repository.</h2><p className="mt-4 text-sm leading-7 text-indigo-100">There is no duplicated web-document copy. Edit the Markdown file under <code className="rounded bg-white/10 px-1.5 py-1">Docs/</code> and the rendered document/search index follows it.</p><Link href="/search" className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-white px-4 text-sm font-extrabold text-[#26358c]">Search all Docs →</Link></aside>
      <div><div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-extrabold tracking-[.17em] text-[var(--lx-primary)]">CANONICAL DOCUMENTS</p><h2 className="mt-2 text-3xl font-black tracking-[-.045em]">Repository source files</h2></div><span className="rounded-full bg-[#eee9ff] px-3 py-1.5 text-xs font-extrabold text-[var(--lx-primary)]">{docs.length} documents</span></div><div className="mt-5 space-y-3">{docs.map((doc,index)=><Link key={doc.relativePath} href={doc.href} className="group block rounded-[22px] border border-[var(--lx-surface)] bg-white p-5 transition hover:-translate-y-0.5 hover:border-[#c6cff0] hover:shadow-[0_14px_30px_rgba(32,52,128,.07)]"><div className="flex gap-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#f0ecff] text-xs font-black text-[var(--lx-primary)]">{String(index+1).padStart(2,"0")}</span><div className="min-w-0 flex-1"><h3 className="break-words text-base font-black">{doc.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-6 text-[var(--lx-muted)]">{doc.excerpt}</p><code className="mt-3 block break-all text-[11px] font-bold text-[#8994b4]">{doc.relativePath}</code><span className="mt-3 inline-flex text-sm font-extrabold text-[var(--lx-secondary)]">Read document →</span></div></div></Link>)}</div></div>
    </section>
  </main></DocsShell>;
}
