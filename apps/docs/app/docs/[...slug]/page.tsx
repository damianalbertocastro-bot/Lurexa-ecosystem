...
export default async function CanonicalDocumentPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  return <DocsShell active={doc.section}><main className="mx-auto max-w-[1480px] px-5 py-9 sm:px-8 sm:py-12">
    <div className="grid gap-8 xl:grid-cols-[240px_minmax(0,1fr)_260px]">
      <aside className="hidden xl:block"><div className="sticky top-28 rounded-[24px] border border-[#dfe6f8] bg-white p-5 shadow-[0_10px_28px_rgba(32,52,128,.05)]"><p className="text-[10px] font-extrabold tracking-[.16em] text-[#7180a8]">CANONICAL SOURCE</p><p className="mt-3 break-words text-xs font-bold leading-5 text-[#53679f]">{doc.relativePath}</p><a href={`/${doc.section.toLowerCase()}`} className="mt-5 inline-flex min-h-11 items-center text-sm font-extrabold text-[#315fd7]">← {doc.section}</a><a href="/search" className="mt-1 inline-flex min-h-11 items-center text-sm font-extrabold text-[#592bd6]">Search Docs →</a></div></aside>

      <section className="min-w-0"><div className="mb-6 flex flex-wrap items-center gap-2 text-xs font-extrabold text-[#7180a8]"><a href="/" className="hover:text-[#315fd7]">Docs</a><span>/</span><a href={`/${doc.section.toLowerCase()}`} className="hover:text-[#315fd7]">{doc.section}</a><span>/</span><span className="text-[#53679f]">{doc.title}</span></div><div className="rounded-[30px] border border-[#dfe6f8] bg-white px-6 py-8 shadow-[0_16px_40px_rgba(32,52,128,.055)] sm:px-9 sm:py-10"><MarkdownDocument content={doc.content} relativePath={doc.relativePath} /></div></section>
