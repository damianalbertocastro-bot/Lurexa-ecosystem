...
export default function BrandIdentityPage() {
  return (
    <main className="mx-auto max-w-6xl px-5 py-12 sm:px-8">
      ...
      <p className="mt-4 max-w-3xl text-sm leading-7 text-[#6677a5]">Use this local Docs route to inspect canonical shared marks and the cross-product recommendation component before an intentional hosted preview or production release.</p>

      <section className="mt-10 rounded-[28px] border border-[#dfe6f8] bg-white p-6 sm:p-8">
        <h2 className="text-xl font-black text-[#071d67]">Master identity</h2>
        <div className="mt-6 flex flex-wrap items-center gap-8"><MasterMark /><MasterMark compact /></div>
      </section>

      <section className="mt-6 rounded-[28px] border border-[#dfe6f8] bg-white p-6 sm:p-8">
        <h2 className="text-xl font-black text-[#071d67]">Current product family</h2>
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((product) => <div key={product} className="rounded-2xl border border-[#e4e9f5] bg-[#fbfcff] p-5"><ProductMark product={product} /><div className="mt-4"><ProductMark product={product} compact /></div></div>)}
        </div>
      </section>

      <section className="mt-6 rounded-[28px] border border-[#dfe6f8] bg-white p-6 sm:p-8">
        <h2 className="text-xl font-black text-[#071d67]">Shared layers and Docs</h2>
        <div className="mt-6 grid gap-4 sm:grid-cols-3"><div className="rounded-2xl border border-[#e4e9f5] p-5"><EcosystemLayerMark layer="core" /></div><div className="rounded-2xl border border-[#e4e9f5] p-5"><EcosystemLayerMark layer="mind" /></div><div className="rounded-2xl border border-[#e4e9f5] p-5"><DocsMark /></div></div>
      </section>

      <section className="mt-6 rounded-[28px] bg-[#071d67] p-6 sm:p-8">
        <h2 className="text-xl font-black text-white">Inverse treatment</h2>
        <div className="mt-6 flex flex-wrap items-center gap-8"><MasterMark inverse />{products.map((product) => <ProductMark key={product} product={product} inverse />)}<DocsMark inverse /><EcosystemLayerMark layer="core" inverse /><EcosystemLayerMark layer="mind" inverse /></div>
      </section>
