...
export function DocsShell({ active, children }: { active?: string; children: React.ReactNode }) {
  ...
  const getSectionLabel = (label: string) => {
    switch (label) {
      case "Architecture": return t("docs.navArchitecture");
      case "Product": return t("docs.navProduct");
      case "Curriculum": return t("docs.navCurriculum");
      case "Engineering": return t("docs.navEngineering");
      case "Governance": return t("docs.navGovernance");
      case "Design": return t("docs.navDesign");
      default: return label;
    }
  };

  return (
    <div className="min-h-screen bg-[var(--lx-surface)] text-[var(--color-brand-navy)]">
      <header className="sticky top-0 z-50 border-b border-[var(--lx-surface)]/90 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1480px] items-center gap-4 px-5 py-3 sm:px-8">
          ...
          <nav className="hidden flex-1 items-center justify-center gap-1 xl:flex" aria-label="Documentation sections">
            {sections.map(([label, href]) => (
              <Link key={label} href={href} aria-current={active === label ? "page" : undefined} className={`rounded-xl px-3 py-2.5 text-sm font-extrabold transition ${active === label ? "bg-[var(--lx-surface)] text-[var(--lx-primary)]" : "text-[var(--lx-muted)] hover:bg-[var(--lx-surface)] hover:text-[var(--color-brand-navy)]"}`}>
        ...
        <nav className="mx-auto flex max-w-[1480px] gap-2 overflow-x-auto px-5 pb-3 xl:hidden" aria-label="Documentation mobile sections">
          {sections.map(([label, href]) => (
            <Link key={label} href={href} aria-current={active === label ? "page" : undefined} className={`shrink-0 rounded-full border px-4 py-2 text-sm font-extrabold ${active === label ? "border-[var(--lx-primary)] bg-[var(--lx-primary)] text-white" : "border-[var(--lx-border)] bg-white text-[var(--lx-primary)]"}`}>
              {getSectionLabel(label)}
            </Link>
          ))}
        </nav>
      ...
      {children}
      <footer className="border-t border-[var(--lx-surface)] bg-white">
        <div className="mx-auto grid max-w-[1480px] gap-6 px-5 py-9 sm:px-8 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            <DocsMark />
            <p className="mt-3 max-w-xl text-sm leading-6 text-[var(--lx-muted)]">A rendered and searchable view over the canonical repository documentation in <code>Docs/</code>.</p>
          </div>
          <div className="text-left md:text-right">
            <Link href="/search" className="text-sm font-extrabold text-[var(--lx-secondary)]">{t("docs.searchDocs")}</Link>
            <p className="mt-2 text-xs font-bold text-[var(--lx-muted)]">Lurexa Learning Technologies · Canonical-source documentation</p>
          </div>
        </div>
      </footer>
    </div>
