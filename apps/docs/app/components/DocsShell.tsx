...
export function DocsShell({ active, children }: { active?: string; children: React.ReactNode }) {
  return <div className="min-h-screen bg-[#f5f7ff] text-[var(--color-brand-navy)]">
    <header className="sticky top-0 z-50 border-b border-[var(--lx-surface)]/90 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1480px] items-center gap-4 px-5 py-3 sm:px-8">
        <Link href="/" className="flex min-h-11 items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--lx-secondary)]" aria-label="Lurexa Docs home">
          ...
          <span><b className="block text-lg font-black tracking-[-.055em]">Lurexa <span className="text-[var(--lx-secondary)]">Docs</span></b><span className="hidden text-[10px] font-extrabold uppercase tracking-[.14em] text-[#8190b7] sm:block">Ecosystem knowledge base</span></span>
        </Link>
        <nav className="hidden flex-1 items-center justify-center gap-1 xl:flex" aria-label="Documentation sections">{sections.map(([label,href])=><Link key={label} href={href} aria-current={active===label?"page":undefined} className={`rounded-xl px-3 py-2.5 text-sm font-extrabold transition ${active===label?"bg-[#eee9ff] text-[var(--lx-primary)]":"text-[#5d6f9d] hover:bg-[#f3f6ff] hover:text-[var(--color-brand-navy)]"}`}>{label}</Link>)}</nav>
        <div className="ml-auto flex items-center gap-2">
          <Link href="/search" aria-label="Search Lurexa Docs" className="inline-flex min-h-11 items-center rounded-xl border border-[#d7e0f6] bg-[#f8faff] px-3.5 text-sm font-extrabold text-[var(--lx-secondary)] transition hover:border-[#b8c7f1] hover:bg-white">
            <span aria-hidden="true">⌕</span>
            <span className="ml-2 hidden sm:inline">Search</span>
          </Link>
          <EcosystemDropdown currentApp="docs" />
          <a href={ecosystemUrl} rel="noreferrer" className="inline-flex min-h-11 items-center rounded-xl border border-[#d7e0f6] bg-white px-3.5 text-sm font-extrabold text-[var(--lx-primary)] shadow-sm transition hover:-translate-y-0.5 hover:border-[#b8c7f1] hover:shadow-md">
            Ecosystem ↗
          </a>
        </div>
      </div>
      <nav className="mx-auto flex max-w-[1480px] gap-2 overflow-x-auto px-5 pb-3 xl:hidden" aria-label="Documentation mobile sections">{sections.map(([label,href])=><Link key={label} href={href} aria-current={active===label?"page":undefined} className={`shrink-0 rounded-full border px-4 py-2 text-sm font-extrabold ${active===label?"border-[var(--lx-primary)] bg-[var(--lx-primary)] text-white":"border-[#d7e0f6] bg-white text-[var(--lx-primary)]"}`}>{label}</Link>)}</nav>
    </header>
    {children}
    <footer className="border-t border-[var(--lx-surface)] bg-white"><div className="mx-auto grid max-w-[1480px] gap-6 px-5 py-9 sm:px-8 md:grid-cols-[1fr_auto] md:items-end"><div><DocsMark/><p className="mt-3 max-w-xl text-sm leading-6 text-[var(--lx-muted)]">A rendered and searchable view over the canonical repository documentation in <code>Docs/</code>.</p></div><div className="text-left md:text-right"><Link href="/search" className="text-sm font-extrabold text-[var(--lx-secondary)]">Search documentation →</Link><p className="mt-2 text-xs font-bold text-[#8a96b5]">Lurexa Learning Technologies · Canonical-source documentation</p></div></div></footer>
  </div>;
}
