  ...
  return <div className="min-h-screen bg-[#f5f7ff] text-[#071d67]">
    <header className="sticky top-0 z-50 border-b border-[#dfe6f8]/90 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1480px] items-center gap-5 px-5 py-3 sm:px-8">
        ...
        <nav className="hidden flex-1 items-center justify-center gap-1 xl:flex" aria-label="Documentation sections">{sections.map(([label,href])=><Link key={label} href={href} aria-current={active===label?"page":undefined} className={`rounded-xl px-3.5 py-2.5 text-sm font-extrabold transition ${active===label?"bg-[#eee9ff] text-[#592bd6]":"text-[#5d6f9d] hover:bg-[#f3f6ff] hover:text-[#071d67]"}`}>{label}</Link>)}</nav>
