...
import { CoachRealtimeAudio } from "./CoachRealtimeAudio";
  ...
  if (!session) return (
    <>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
        <article className="rounded-[34px] border border-[var(--lx-border)] bg-white p-8 shadow-[0_20px_60px_rgba(39,61,132,.08)]">
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-black tracking-[.18em] text-[var(--lx-primary)]">ADAPTIVE SPEAKING SESSION</p>
            <button
              type="button"
              onClick={() => setShowPreflight(true)}
              className="flex items-center gap-1.5 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100 transition"
            >
              <span>🎙️</span>
              <span>Test Audio & Mic</span>
            </button>
          </div>
          <h1 className="mt-3 text-4xl font-black tracking-[-.055em] sm:text-5xl text-slate-950">Continue from where you are.</h1>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-700">
            Coach requests your authorized learner context when the session starts, then adapts conversation, pronunciation, fluency, and feedback around it.
          </p>
          {error ? <p role="alert" className="mt-5 rounded-2xl bg-rose-50 p-4 text-sm font-bold text-rose-800">{error}</p> : null}
          <Button type="button" disabled={busy} onClick={handleStartClick} className="mt-8 min-h-14 rounded-2xl bg-gradient-to-br from-[var(--lx-primary)] to-[var(--lx-secondary)] px-8 text-sm font-black text-white shadow-lg disabled:opacity-60">
            {busy ? "Connecting your context…" : "Start speaking session →"}
          </Button>
        </article>
        <aside className="rounded-[34px] bg-[var(--color-brand-navy)] p-8 text-white">
          <p className="text-[10px] font-black tracking-[.18em] text-[var(--lx-accent)]">COACH BOUNDARY</p>
          <h2 className="mt-3 text-2xl font-black">Coach owns practice. Core owns the trusted record.</h2>
          <p className="mt-4 text-sm leading-7 text-indigo-200">Coach interprets and practices with authorized context. It does not become the authority for placement, qualification, course enrollment, or persistence.</p>
        </aside>
      </section>
    </>
  );

  return <section className="grid gap-6 xl:grid-cols-[.72fr_1.28fr] xl:items-start"><aside className="space-y-4 xl:sticky xl:top-28"><article className="rounded-[28px] border border-[var(--lx-border)] bg-white p-6"><p className="text-[10px] font-black tracking-[.17em] text-[var(--lx-primary)]">SESSION CONTEXT</p><div className="mt-4 flex flex-wrap gap-2">{contextItems.map((item) => <span key={item} className="rounded-full border border-[var(--lx-border)] bg-[var(--lx-surface)] px-3 py-1.5 text-xs font-bold text-[var(--lx-primary)]">{item}</span>)}</div></article>{cue ? <article className="rounded-[28px] border border-amber-200 bg-amber-50 p-6"><p className="text-[10px] font-black tracking-[.17em] text-amber-800">COACHING TIP</p><p className="mt-2 text-sm font-semibold leading-6 text-amber-950">{cue}</p></article> : null}{diagnosticReport ? <article className="rounded-[28px] border border-sky-200 bg-sky-50 p-6"><p className="text-[10px] font-black tracking-[.17em] text-sky-800">ACOUSTIC &amp; L1 DIAGNOSTIC</p><p className="mt-2 text-xs font-bold text-sky-900">Score: {Math.round((diagnosticReport.score ?? 0.85) * 100)}% Intelligibility</p>{diagnosticReport.patterns?.length ? <div className="mt-2 flex flex-wrap gap-1">{diagnosticReport.patterns.map((p) => <span key={p} className="rounded-md bg-white border border-sky-300 px-2 py-0.5 text-[10px] font-bold text-sky-800">{p}</span>)}</div> : null}{diagnosticReport.remedialAction ? <p className="mt-2 text-[11px] text-sky-950 font-medium">{diagnosticReport.remedialAction}</p> : null}</article> : null}<article className="rounded-[28px] border border-[var(--lx-accent)] bg-[var(--lx-surface)] p-6"><p className="text-[10px] font-black tracking-[.17em] text-[var(--lx-success)]">EVIDENCE BOUNDARY</p><p className="mt-2 text-xs leading-5 text-[var(--lx-muted)]">Learner-mode sessions can contribute minimized learning evidence. Educator-professional sessions stay in professional evidence and do not enter the ordinary learner evidence pipeline.</p></article><Button type="button" disabled={busy || ending} onClick={() => void finish()} className="w-full min-h-12 rounded-xl bg-[var(--color-brand-navy)] px-5 text-sm font-black text-white disabled:opacity-50">{ending ? "Saving & validating return…" : `${returnLabel} →`}</Button></aside>
    <article className="overflow-hidden rounded-[34px] border border-[var(--lx-border)] bg-white shadow-[0_24px_70px_rgba(39,61,132,.1)]"><header className="flex items-center justify-between border-b border-[var(--lx-border)] px-6 py-5"><div><p className="text-[10px] font-black tracking-[.17em] text-[var(--lx-primary)]">LIVE PRACTICE</p><h2 className="mt-1 text-xl font-black">{session.mode === "educator_professional" ? "Professional English Coach" : "Adaptive Speaking Coach"}</h2></div><span className="rounded-full bg-[var(--lx-surface)] px-3 py-1.5 text-xs font-black text-[var(--lx-success)]">Active</span></header><div className="p-4 border-b border-[var(--lx-border)] bg-slate-50/50"><CoachRealtimeAudio sessionId={session.id} onTranscriptTurn={(sender, text) => { setSession((prev) => { if (!prev) return null; return { ...prev, transcript: [...prev.transcript, { sender, text, timestamp: new Date().toISOString() }] }; }); }} onPhonemeDiagnostics={(diag) => { setDiagnosticReport(diag); }} /></div><div className="min-h-[390px] max-h-[540px] space-y-4 overflow-y-auto bg-gradient-to-b from-[var(--lx-surface)] to-[var(--lx-surface)] px-5 py-7 sm:px-8">{session.transcript.map((message, index) => <Bubble key={`${message.timestamp}-${index}`} sender={message.sender} text={message.text} />)}{busy ? <div className="text-xs italic text-[var(--lx-muted)]">Coach is preparing feedback…</div> : null}<div ref={bottomRef}/></div><div className="border-t border-[var(--lx-border)] bg-[var(--lx-surface)] px-6 py-3"><div className="flex flex-wrap gap-2">{starters.map((starter) => <Button key={starter} type="button" disabled={busy || ending} onClick={() => void send(starter)} className="rounded-xl border border-indigo-100 bg-white px-3 py-1.5 text-xs font-semibold text-indigo-700 disabled:opacity-40">+ {starter}</Button>)}</div></div><footer className="border-t border-[var(--lx-border)] p-5"><div className="flex gap-3"><Input value={input} disabled={ending} onChange={(event)=>setInput(event.target.value)} onKeyDown={(event)=>{ if (event.key === "Enter") void send(); }} placeholder="Type your response in English…" className="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-sm outline-none focus:border-indigo-600"/><Button type="button" disabled={!input.trim() || busy || ending} onClick={() => void send()} className="rounded-2xl bg-indigo-600 px-6 text-sm font-black text-white disabled:opacity-40">Send</Button></div>{error ? <p role="alert" className="mt-3 text-xs font-bold text-rose-700">{error}</p> : null}</footer></article></section>;
}
