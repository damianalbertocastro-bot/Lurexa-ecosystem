...
export default function PracticePage() {
  ...
  if (!session) return <section className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]"><article className="rounded-[34px] border border-[var(--lx-border)] bg-white p-8 shadow-[0_20px_60px_rgba(39,61,132,.08)]"><p className="text-[10px] font-black tracking-[.18em] text-[var(--lx-primary)]">ADAPTIVE SPEAKING SESSION</p><h1 className="mt-3 text-4xl font-black tracking-[-.055em] sm:text-5xl text-slate-950">Continue from where you are.</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-slate-700">Coach requests your authorized learner context when the session starts, then adapts conversation, pronunciation, fluency, and feedback around it.</p>{error ? <p role="alert" className="mt-5 rounded-2xl bg-rose-50 p-4 text-sm font-bold text-rose-800">{error}</p> : null}<Button type="button" disabled={busy} onClick={() => void start()} className="mt-8 min-h-14 rounded-2xl bg-gradient-to-br from-[var(--lx-primary)] to-[var(--lx-secondary)] px-8 text-sm font-black text-white shadow-lg disabled:opacity-60">{busy ? "Connecting your context…" : "Start speaking session →"}</Button></article><aside className="rounded-[34px] bg-[var(--color-brand-navy)] p-8 text-white"><p className="text-[10px] font-black tracking-[.18em] text-[var(--lx-accent)]">COACH BOUNDARY</p><h2 className="mt-3 text-2xl font-black">Coach owns practice. Core owns the trusted record.</h2><p className="mt-4 text-sm leading-7 text-indigo-200">Coach interprets and practices with authorized context. It does not become the authority for placement, qualification, course enrollment, or persistence.</p></aside></section>;
