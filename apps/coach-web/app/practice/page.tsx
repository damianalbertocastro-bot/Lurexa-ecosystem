...
export default function PracticePage() {
  ...
  async function send(textToSend?: string) {
    ...
    try {
      const response = await authenticatedFetch("/api/coach", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "sendTurn", sessionId: session.id, message }) });
      const payload = await response.json() as { session?: CoachSession; coachingCue?: string; error?: string };
      if (!response.ok || !payload.session) throw new Error(payload.error ?? "Unable to send your Coach turn.");
      setSession(payload.session); setCue(payload.coachingCue ?? null);
