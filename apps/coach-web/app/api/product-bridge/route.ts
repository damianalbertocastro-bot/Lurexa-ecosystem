...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    return Response.json(await createProductBridge({ actorId: actor.uid, learnerId: actor.uid, source: "coach", destination: body.destination, purpose: body.purpose, destinationRef: body.destinationRef, ...(body.contextRef ? { contextRef: body.contextRef } : {}), singleUse: true }), { headers: { "Cache-Control": "private, no-store, max-age=0" } });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unable to process Product Bridge.";
    const status = message === "Authentication is required." ? 401 : message.includes("not authorized") || message.includes("identity") ? 403 : message.includes("expired") || message.includes("already been used") ? 410 : 400;
