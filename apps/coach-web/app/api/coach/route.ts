...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (body.action === "sendTurn") {
      ...
      return Response.json(await CoachPlatformService.sendTurn(actor, { sessionId: body.sessionId, message: body.message, ...(body.audioDurationMs !== undefined ? { audioDurationMs: body.audioDurationMs } : {}) }), { headers: { "Cache-Control": "private, no-store, max-age=0" } });
    }
    if (body.action === "endSession") {
      if (!body.sessionId) throw new Error("sessionId is required for ending a Coach session.");
      return Response.json(await endCoachSession(actor, { sessionId: body.sessionId }), { headers: { "Cache-Control": "private, no-store, max-age=0" } });
    }
    if (body.action === "resumeSession") {
      if (!body.sessionId) throw new Error("sessionId is required for resuming a Coach session.");
      return Response.json(await resumeCoachSession(actor, { sessionId: body.sessionId }), { headers: { "Cache-Control": "private, no-store, max-age=0" } });
    }
