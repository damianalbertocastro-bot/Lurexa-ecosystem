...
].map(([title, copy]) => <article key={title} className="rounded-[26px] border border-[var(--lx-border)] bg-white p-6"><h2 className="text-xl font-black">{title}</h2><p className="mt-3 text-sm leading-6 text-[var(--lx-muted)]">{copy}</p></article>)}</section><div className="mt-10 rounded-[28px] border border-cyan-200 bg-cyan-50 p-6"><b className="text-sm text-cyan-950">Current implementation note</b><p className="mt-2 text-sm leading-6 text-cyan-900">Until the governed Coach-history projection is exposed by Core, this screen intentionally shows no fabricated session list or invented progress metrics.</p></div><Link href="/practice" className="mt-8 inline-flex min-h-12 items-center rounded-xl bg-[var(--color-brand-navy)] px-6 text-sm font-black text-white">Start a new session →</Link></main></CoachShell>;
}
