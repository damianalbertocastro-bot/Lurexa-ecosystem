...
export default function CoachDashboard() {
  ...
  return <CoachShell active="Dashboard"><main className="mx-auto max-w-[1320px] px-5 py-12 sm:px-8">
    ...
    <section className="mt-12 rounded-[30px] border border-[#dce8f5] bg-white p-6 shadow-[0_14px_40px_rgba(39,61,132,.06)] sm:p-8">
      <p className="text-[10px] font-black tracking-[.16em] text-[#6b2bd9]">ONE LUREXA ACCOUNT</p>
      <div className="mt-3 grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
        <div><h2 className="text-2xl font-black tracking-[-.04em]">Move across your Lurexa learning ecosystem.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#6074a5]">Your Lurexa identity is shared across products. Existing learners should sign in with the same account rather than create a second identity.</p></div>
        <div className="flex flex-wrap gap-2">
          <a href={urls.learn} className="inline-flex min-h-11 items-center rounded-xl border border-[#d7e0f6] px-4 text-sm font-black text-[#315fd7]">Open Learn →</a>
          <a href={urls.teach} className="inline-flex min-h-11 items-center rounded-xl border border-[#d7e0f6] px-4 text-sm font-black text-[#315fd7]">Open Teach →</a>
          <a href={urls.ecosystem} className="inline-flex min-h-11 items-center rounded-xl bg-[#071d67] px-4 text-sm font-black text-white">All Lurexa products →</a>
        </div>
      </div>
    </section>
  </main></CoachShell>;
