  ...
  return (
    <div className="space-y-3 rounded-xl border border-[#dfe7fb] bg-[var(--learn-canvas)] p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-[#071d67]">Learning activities</p>
      ...
      {drafts.map((draft, index) => (
        <div key={draft.id} className="space-y-3 rounded-xl border border-[#dfe7fb] bg-white p-3">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-[#071d67]">Activity {index + 1}</p>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <label className="text-sm font-medium text-[#314b88]">Activity type
              <select className="mt-1 w-full rounded-xl border border-[#d7e0f6] p-2 text-[#071d67]" value={draft.type} onChange={(event) => onChange(updateDraft(drafts, draft.id, { type: event.target.value as LearningActivityType }))}>
            ...
            <label className="text-sm font-medium text-[#314b88]">Lesson stage
              <select className="mt-1 w-full rounded-xl border border-[#d7e0f6] p-2 text-[#071d67]" value={draft.stage} onChange={(event) => onChange(updateDraft(drafts, draft.id, { stage: event.target.value as LessonStage }))}>
          ...
          {draft.type !== "short_response" ? <>
            <label className="block text-sm font-medium text-[#314b88]">Options, one per line
              <textarea className="mt-1 w-full rounded-xl border border-[#d7e0f6] p-3 text-[#071d67]" value={draft.options} onChange={(event) => onChange(updateDraft(drafts, draft.id, { options: event.target.value }))} rows={4} />
            </label>
            <label className="block text-sm font-medium text-[#314b88]">Correct answer{draft.type === "multiple_selection" ? "s, one per line" : ""}
              <textarea className="mt-1 w-full rounded-xl border border-[#d7e0f6] p-3 text-[#071d67]" value={draft.correctAnswers} onChange={(event) => onChange(updateDraft(drafts, draft.id, { correctAnswers: event.target.value }))} rows={draft.type === "multiple_selection" ? 3 : 1} />
            </label>
          </> : null}
