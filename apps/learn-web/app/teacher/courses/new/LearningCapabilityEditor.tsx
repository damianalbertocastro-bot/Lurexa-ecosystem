...
export function LearningCapabilityEditor({ drafts, onChange }: Props) {
  return (
    <div className="space-y-4 rounded-xl border border-[#dfe7fb] bg-[#f8faff] p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-[#071d67]">Advanced learning capabilities</p>
          <p className="mt-1 max-w-2xl text-xs leading-5 text-[#5d6f9d]">Author approved listening, recorded speaking, and AI roleplay blocks. Provider settings stay server-controlled; the server validates and sanitizes every capability before persistence.</p>
        </div>
      ...
      {drafts.map((draft, index) => (
        <div key={draft.blockId} className="space-y-3 rounded-xl border border-[#dfe7fb] bg-white p-4">
          ...
          <label className="flex items-center gap-2 text-sm font-medium text-[#314b88]"><input type="checkbox" checked={draft.required} onChange={(event) => onChange(updateDraft(drafts, draft.blockId, { required: event.target.checked }))} />Required in this lesson</label>

          {draft.kind === "model_listening" ? <>
            <label className="block text-sm font-medium text-[#314b88]">Approved model text<textarea className="mt-1 w-full rounded-xl border border-[#d7e0f6] p-3 text-[#071d67]" rows={5} value={draft.modelText} onChange={(event) => onChange(updateDraft(drafts, draft.blockId, { modelText: event.target.value }))} /></label>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><Input label="Locale" value={draft.locale} onChange={(event) => onChange(updateDraft(drafts, draft.blockId, { locale: event.target.value }))} /><label className="text-sm font-medium text-[#314b88]">Playback goal<select className="mt-1 w-full rounded-xl border border-[#d7e0f6] p-2 text-[#071d67]" value={draft.playbackGoal} onChange={(event) => onChange(updateDraft(drafts, draft.blockId, { playbackGoal: event.target.value as ModelListeningCapability["playbackGoal"] }))}><option value="meaning">Meaning</option><option value="noticing">Noticing</option><option value="pronunciation_model">Pronunciation model</option></select></label><Input label="Optional approved audio URL" value={draft.audioUrl} onChange={(event) => onChange(updateDraft(drafts, draft.blockId, { audioUrl: event.target.value }))} /></div>
          </> : null}
