        ...
        {loading ? <p className="text-[#6677a5]">Loading courses...</p> : courses.length === 0 ? <Card title="No courses yet"><p className="text-[#6677a5]">Create a course, then add modules and lessons in the course builder.</p></Card> : courses.map(({ course, lessons }) => (
          <Card key={course.id} title={course.title} subtitle={`Last updated ${new Date(course.updatedAt).toLocaleString()}`}>
            ...
            <div className="mt-4 divide-y divide-[#edf1fb] rounded-xl border border-[#dfe7fb] px-4">
              {lessons.length === 0 ? <p className="py-3 text-sm text-[#6677a5]">No lessons yet.</p> : lessons.map(({ moduleTitle, lesson }) => <div key={lesson.id} className="flex items-center justify-between gap-3 py-3 text-sm"><span><span className="font-medium text-[#071d67]">{lesson.title}</span><span className="text-[#6677a5]"> · {moduleTitle} · {lesson.estimatedMinutes} min · {lesson.contentBlocks.filter((block) => block.type === "interactive" || block.type === "quiz_embed").length} activities</span></span><span className="flex gap-2"><Button variant="ghost" size="sm" onClick={() => openLessonEditor(lesson)}>Edit text</Button><Button variant="secondary" size="sm" onClick={() => router.push(`/teacher/courses/new?courseId=${encodeURIComponent(course.id)}&lessonId=${encodeURIComponent(lesson.id)}`)}>Edit activities</Button></span></div>)}
            </div>
          </Card>
        ))}
      </div>
