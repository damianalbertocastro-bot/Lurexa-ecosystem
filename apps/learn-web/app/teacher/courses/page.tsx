  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        {loading ? <p className="text-[var(--lx-muted)]">Loading courses...</p> : courses.length === 0 ? <Card title="No courses yet"><p className="text-[var(--lx-muted)]">Create a course, then add modules and lessons in the course builder.</p></Card> : courses.map(({ course, lessons }) => (
          <Card key={course.id} title={course.title} subtitle={`Last updated ${new Date(course.updatedAt).toLocaleString()}`}>
            <div className="flex flex-wrap items-center gap-3 pt-2"><Badge variant={course.status === "published" ? "success" : "warning"}>{course.status}</Badge><span className="text-sm text-[var(--lx-muted)]">{course.description}</span><Button variant="secondary" size="sm" onClick={() => openCourseEditor(course)}>Edit course</Button></div>
            <div className="mt-4 divide-y divide-[var(--lx-surface)] rounded-xl border border-[var(--lx-border)] px-4">
              {lessons.length === 0 ? <p className="py-3 text-sm text-[var(--lx-muted)]">No lessons yet.</p> : lessons.map(({ moduleTitle, lesson }) => <div key={lesson.id} className="flex items-center justify-between gap-3 py-3 text-sm"><span><span className="font-medium text-[var(--color-brand-navy)]">{lesson.title}</span><span className="text-[var(--lx-muted)]"> · {moduleTitle} · {lesson.estimatedMinutes} min · {lesson.contentBlocks.filter((block) => block.type === "interactive" || block.type === "quiz_embed").length} activities</span></span><span className="flex gap-2"><Button variant="ghost" size="sm" onClick={() => openLessonEditor(lesson)}>Edit text</Button><Button variant="secondary" size="sm" onClick={() => router.push(`/teacher/courses/new?courseId=${encodeURIComponent(course.id)}&lessonId=${encodeURIComponent(lesson.id)}`)}>Edit activities</Button></span></div>)}
            </div>
          </Card>
        ))}
      </div>
      ...
      <Modal isOpen={editingLesson !== null} onClose={() => setEditingLesson(null)} title="Edit lesson"><form onSubmit={handleUpdateLesson} className="space-y-4"><Input label="Lesson title" value={lessonTitle} onChange={(event) => setLessonTitle(event.target.value)} required /><label className="block text-sm font-medium text-[var(--lx-muted)]">Lesson content<textarea className="mt-1 w-full rounded-xl border border-[var(--lx-border)] p-3 text-[var(--color-brand-navy)]" value={lessonContent} onChange={(event) => setLessonContent(event.target.value)} rows={8} required /></label><Button type="submit" className="w-full" isLoading={saving}>Save lesson changes</Button></form></Modal>
    </>
  );
}
