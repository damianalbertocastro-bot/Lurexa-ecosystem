...
export default function TeacherStudentsPage() {
  ...
  return <div className="min-h-screen bg-slate-50 p-4 sm:p-8"><div className="mx-auto max-w-4xl space-y-6">
    <div className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-3xl font-bold text-slate-900">Students</h1><p className="text-slate-500">Invite learners and track active access.</p></div><Button variant="secondary" onClick={() => router.push("/teacher/dashboard")}>Back to dashboard</Button></div>
    ...
    <Card title="Active student invitations" subtitle="A student becomes active after accepting an invitation">{loading ? <p className="text-slate-500">Loading students...</p> : invitations.length === 0 ? <p className="text-slate-500">No active invitations yet.</p> : <div className="divide-y divide-slate-100">{invitations.map((invitation) => <div key={invitation.id} className="flex items-center justify-between py-3"><div><p className="font-medium text-slate-900">{invitation.email}</p><p className="text-xs text-slate-500">Code: {invitation.code}</p></div><Badge variant={invitation.usedAt ? "success" : "info"}>{invitation.usedAt ? "Active" : "Invited"}</Badge></div>)}</div>}</Card>
  </div></div>;
}
