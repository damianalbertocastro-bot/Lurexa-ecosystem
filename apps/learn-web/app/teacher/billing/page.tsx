...
import { PLAN_CONFIGS } from "@lurexa/backend";
import type { OrganizationPlanProjection, PricingPlan } from "@lurexa/types";
import { authenticatedFetch } from "../../../lib/authenticated-fetch";

export default function TeacherBillingPage() {
  const [projection, setProjection] = useState<OrganizationPlanProjection | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadPlan() {
      try {
        const response = await authenticatedFetch("/api/teacher/plan");
        const payload = await response.json() as OrganizationPlanProjection & { error?: string };
        if (!response.ok) throw new Error(payload.error ?? "Unable to load plan details.");
        if (active) setProjection(payload);
      } catch (cause) {
        if (active) setError(cause instanceof Error ? cause.message : "Unable to load plan details.");
      } finally {
  ...
  const currentPlan = projection?.plan ?? "free";
  const limits = projection?.limits ?? PLAN_CONFIGS.free;
  const studentSeats = projection?.studentSeats ?? 0;
  ...
  return (
    <main className="mx-auto max-w-[1180px] px-5 py-8 sm:px-8 sm:py-12">
      <section className="relative overflow-hidden rounded-[34px] bg-gradient-to-br from-[#071d67] via-[#17368f] to-[#592bd6] px-6 py-10 text-white shadow-[0_22px_60px_rgba(35,48,133,.18)] sm:px-10 sm:py-12">
        ...
        <div className="relative max-w-3xl"><p className="text-[11px] font-extrabold tracking-[.2em] text-[#8df4ef]">ORGANIZATION PLAN · LUREXA LEARN</p><h1 className="mt-4 text-4xl font-black tracking-[-.055em] sm:text-5xl">Understand the limits currently applied.</h1><p className="mt-4 max-w-2xl text-base leading-7 text-indigo-100">This surface reports trusted organization membership, stored plan/subscription state, and observed seat usage. Paid checkout is not exposed until commerce is actually configured.</p>{projection ? <p className="mt-4 text-sm font-bold text-indigo-100">{projection.organizationName} · refreshed {new Date(projection.generatedAt).toLocaleString()}</p> : null}</div>
      </section>

      {loading ? <div role="status" aria-live="polite" className="mt-6 rounded-[26px] border border-[#dfe6f8] bg-white p-7 text-[#6677a5]">Loading plan details…</div> : null}
      ...
      {!loading && !error && projection ? <>
        <section className="mt-6 grid gap-4 md:grid-cols-3">
          ...
          <article className="rounded-[26px] border border-[#cfeee9] bg-[#e9fbf9] p-6 shadow-[0_12px_30px_rgba(32,52,128,.05)]"><p className="text-[10px] font-extrabold tracking-[.15em] text-[#137d7f]">OFFLINE SUPPORT</p><div className="mt-4"><Badge variant={limits.offlineSupport ? "success" : "warning"}>{limits.offlineSupport ? "Enabled" : "Not included"}</Badge></div><p className="mt-3 text-xs leading-5 text-[#477a76]">Capability follows the active stored plan configuration.</p></article>
        </section>

        <section className="mt-6 rounded-[30px] border border-[#dfe6f8] bg-white p-7 shadow-[0_14px_36px_rgba(32,52,128,.07)] sm:p-8">
          <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-extrabold tracking-[.18em] text-[#592bd6]">PLAN CONFIGURATION</p><h2 className="mt-2 text-2xl font-black tracking-[-.04em] text-[#10245f]">Current plan: {currentPlan}</h2><p className="mt-2 text-sm text-[#6677a5]">Subscription status: {projection.subscriptionStatus ?? "No paid subscription record"}</p></div><Badge variant={currentPlan === "free" ? "default" : "success"}>{currentPlan.toUpperCase()}</Badge></div>
