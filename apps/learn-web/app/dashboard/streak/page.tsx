...
export default function StreakPage() {
  ...
  return <div className="min-h-screen bg-slate-50 p-4 sm:p-8"><div className="mx-auto max-w-3xl space-y-6"><div className="flex items-center justify-between border-b border-slate-200 pb-6"><div><h1 className="text-3xl font-bold text-slate-900">Your learning streak</h1><p className="text-slate-500">Small, consistent learning sessions build progress.</p></div><Button variant="secondary" onClick={() => router.push("/dashboard")}>Back to dashboard</Button></div><Card title="Current streak" subtitle="Complete lessons on consecutive days to keep it going."><div className="py-6 text-center"><p className="text-6xl">🔥</p><p className="mt-3 text-4xl font-bold text-amber-600">{summary?.streakDays ?? 0} days</p><Badge variant="warning">Last activity: {summary?.lastActivityAt ? new Date(summary.lastActivityAt).toLocaleDateString() : "Start a lesson today"}</Badge></div></Card><Card title="How streaks work"><p className="text-sm leading-6 text-slate-600">A day counts when a lesson is completed. Keep learning each day to grow the streak; after a missed day, the next completed lesson starts a new one.</p></Card></div></div>;
}
