...
export function LessonRuntime({ courseId, lessonId }: { courseId: string; lessonId: string }) {
  ...
  async function submitBlock(blockId: string, kind: "quiz" | "activity", activity?: LearnerLearningActivity) {
    ...
    if (!answers.length) {
      setFeedback((current) => ({ ...current, [blockId]: { passed: false, message: "Choose an answer before submitting." } }));
      return;
    }
    ...
    try {
      ...
      setFeedback((current) => ({ ...current, [blockId]: { passed: result.attempt.passed, message } }));
      ...
      setFeedback((current) => ({ ...current, [blockId]: { passed: false, message: caught instanceof Error ? caught.message : "Unable to submit this activity." } }));
    } finally {
  ...
  async function submitShortResponse(blockId: string) {
    ...
    if (responseText.length < 8) {
      setFeedback((current) => ({ ...current, [blockId]: { passed: false, message: "Write a little more before submitting." } }));
      return;
    }
    setSubmittingId(blockId);
    try {
      ...
      const result = body as { attempt: StudentProgress["attempts"][number]; explanation: string | null };
      setFeedback((current) => ({ ...current, [blockId]: { passed: true, message: result.explanation ?? "Your response was saved as learning evidence." } }));
    } catch (caught) {
      setFeedback((current) => ({ ...current, [blockId]: { passed: false, message: caught instanceof Error ? caught.message : "Unable to submit this response." } }));
    } finally {
      setSubmittingId(null);
    }
  }
  ...
  if (loading) return <div className="mx-auto max-w-3xl px-6 py-16 text-slate-500">Loading lesson…</div>;
    ...
    return (
      <div className="mx-auto max-w-3xl px-6 py-16">
        <div className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          ...
          <button className="mt-6 rounded-xl bg-indigo-600 px-5 py-3 font-semibold text-white" onClick={() => router.push("/dashboard")}>Back to dashboard</button>
        </div>
      </div>
    );
  }
