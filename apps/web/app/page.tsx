    ...
    <main className={styles.page}>
      ...
      <header className={styles.navWrapper}>
        <nav className={styles.nav} aria-label="Lurexa ecosystem navigation">
          ...
          <div className={styles.navLinks}>
            <a href="#why-lurexa">{t("landing.nav.whyLurexa", "Why Lurexa")}</a>
            <a href="#learners">{t("landing.nav.learners", "Learners")}</a>
            <a href="#educators">{t("landing.nav.educators", "Educators")}</a>
            <a href="#institutions">{t("landing.nav.institutions", "Institutions")}</a>
            <a href="#pricing">{t("landing.nav.pricing", "Pricing")}</a>
            <a href="#shared-intelligence">{t("landing.nav.architecture", "Architecture")}</a>
          </div>
          ...
          >
            ...
            <div className={styles.accordionContainer}>
              ...
              <div className={styles.accordionItem}>
                ...
                >
                  <span>{t("landing.sidebar.productsTitle", "🚀 Products & Ecosystem")}</span>
              ...
              <div className={styles.accordionItem}>
                ...
                >
                  <span>{t("landing.sidebar.navTitle", "🧭 Navigation & Pillars")}</span>
              ...
              <div className={styles.accordionItem}>
                ...
                >
                  <span>{t("landing.sidebar.plansTitle", "💳 Plans & Memberships")}</span>
                ...
                {openAccordion === "pricing" && (
                  <div className={styles.accordionContent}>
                    <a href="#pricing" className={styles.accordionLink} onClick={() => { setPricingTab("individual"); setSidebarOpen(false); }}>
                      {t("landing.pricing.tabIndividual", "👤 For Individual Learners")}
                    </a>
                    <a href="#pricing" className={styles.accordionLink} onClick={() => { setPricingTab("institutional"); setSidebarOpen(false); }}>
                      {t("landing.pricing.tabInstitutional", "🏢 For Companies & Institutions")}
                    </a>
                  </div>
                )}
              </div>
            ...
            <div className={styles.sidebarFooter}>
              <a className={styles.primaryCta} href={learnUrl} style={{ width: "100%", justifyContent: "center" }}>
                {t("landing.sidebar.launchWorkspace", "Launch Learning Workspace ↗")}
              </a>
            </div>
          </aside>
        </div>
      ...
      <section id="top" className={styles.hero}>
        <div className={styles.heroCopy}>
          <p className={styles.kicker}>{t("landing.hero.kicker", "LUREXA LEARNING TECHNOLOGIES")}</p>
          <h1>
            {t("landing.hero.titleBefore", "Learning grows when ")}
            <em>{t("landing.hero.titleHighlight", "everything connects.")}</em>
          </h1>
          <p className={styles.intro}>
            {t("landing.hero.subtitle", "Lurexa is an intelligent English learning ecosystem where each product understands the learner, preserves persistent pedagogical evidence, and accelerates conversational fluency.")}
          </p>
          <div className={styles.heroActions}>
            <a className={styles.primaryCta} href="#why-lurexa" aria-label="Why Lurexa">
              {t("landing.hero.whyLurexaCta", "Why Lurexa")} <span>↓</span>
            </a>
            <a className={styles.textCta} href="#pricing">
              {t("landing.hero.pricingCta", "View Plans & Pricing")} <span>→</span>
            </a>
          </div>
          <p className={styles.microcopy}>{t("landing.hero.microcopy", "One learner. One evolving model. Every experience adapts around it.")}</p>
        </div>
        <div className={styles.orbit} aria-label="Interactive Lurexa product map">
          <div className={styles.orbitCore}>
            <MasterMark compact size="lg" />
            <span>
              {t("landing.hero.orbitLearn", "Learn.")}<br/>
              {t("landing.hero.orbitConnect", "Connect.")}<br/>
              <b>{t("landing.hero.orbitGrow", "Grow.")}</b>
            </span>
          </div>
      ...
      <section id="shared-intelligence" className={styles.trustBar} aria-label="Lurexa shared architecture">
        <div>
          <span className={styles.stepNumber}>{t("landing.trustBar.step1Num", "01")}</span>
          <b>{t("landing.trustBar.step1Title", "Lurexa Core")}</b>
          <p>{t("landing.trustBar.step1Desc", "Protects identity, access, and trusted persistent learner records.")}</p>
        </div>
        <span className={styles.flow}>→</span>
        <div>
          <span className={styles.stepNumber}>{t("landing.trustBar.step2Num", "02")}</span>
          <b>{t("landing.trustBar.step2Title", "Lurexa Mind")}</b>
          <p>{t("landing.trustBar.step2Desc", "Interprets authorized speech, quizzes, and CEFR progression.")}</p>
        </div>
        <span className={styles.flow}>→</span>
        <div>
          <span className={styles.stepNumber}>{t("landing.trustBar.step3Num", "03")}</span>
          <b>{t("landing.trustBar.step3Title", "Focused Products")}</b>
          <p>{t("landing.trustBar.step3Desc", "Deliver tailored experiences for learners, coaches, and educators.")}</p>
        </div>
      </section>
      ...
      <section id="why-lurexa" className={styles.whySection} aria-labelledby="why-heading">
        <div className={styles.sectionHeading}>
          <p className={styles.kicker}>{t("landing.whyLurexa.kicker", "THE LUREXA ADVANTAGE")}</p>
          <h2 id="why-heading">
            {t("landing.whyLurexa.headingBefore", "Why Lurexa is ")}
            <em>{t("landing.whyLurexa.headingHighlight", "different from the rest.")}</em>
          </h2>
          <p>{t("landing.whyLurexa.lead", "Generic language apps rely on isolated flashcards and superficial gamification. Lurexa unites contrastive linguistics, authentic speech intelligence, and teacher synergy into one unified ecosystem.")}</p>
        </div>

        <div className={styles.whyGrid}>
          <article className={styles.whyCard}>
            <div className={styles.whyIcon}>🇩🇴 🎙️</div>
            <h3>{t("landing.whyLurexa.card1Title", "Dominican Spanish Linguistic Transfer")}</h3>
            <p>{t("landing.whyLurexa.card1Desc", "Unlike one-size-fits-all tools, Coach integrates deep contrastive phonology tailored for Dominican Spanish speakers—addressing /s/ aspiration, consonant codas, and vowel durations without accent erasure.")}</p>
          </article>

          <article className={styles.whyCard}>
            <div className={styles.whyIcon}>🧠 ⚡</div>
            <h3>{t("landing.whyLurexa.card2Title", "Continuous Single Learner Model")}</h3>
            <p>{t("landing.whyLurexa.card2Desc", "Your lessons in Learn, oral drills in Coach, and teacher evaluations in Teach feed into a single persistent learner model in Core. Learning never starts over.")}</p>
          </article>

          <article className={styles.whyCard}>
            <div className={styles.whyIcon}>👩‍🏫 📈</div>
            <h3>{t("landing.whyLurexa.card3Title", "Teacher-Empowering Synergy")}</h3>
            <p>{t("landing.whyLurexa.card3Desc", "Lurexa doesn't replace teachers—it amplifies them. Lurexa Teach equips educators with verifiable CEFR micro-credentials, classroom mastery diagnostics, and collaborative cohorts.")}</p>
          </article>

          <article className={styles.whyCard}>
            <div className={styles.whyIcon}>🛡️ 🏛️</div>
            <h3>{t("landing.whyLurexa.card4Title", "Institutional Trust & Governance")}</h3>
            <p>{t("landing.whyLurexa.card4Desc", "Lurexa Core enforces strict trust boundaries. Student evidence, AI interpretations, and institutional metrics are governed with verifiable audit trails and enterprise security.")}</p>
          </article>
        </div>

        <div className={styles.whyCtaBanner}>
          <div>
            <h3>{t("landing.whyLurexa.bannerHeading", "Ready to accelerate your fluency or transform your classroom?")}</h3>
            <p>{t("landing.whyLurexa.bannerDesc", "Explore our flexible plans designed for individual learners, educators, and enterprise institutions.")}</p>
          </div>
          <a className={styles.primaryCta} href="#pricing">
            {t("landing.whyLurexa.bannerCta", "Know More About Our Plans & Products")} <span>→</span>
          </a>
        </div>
      </section>
      ...
      <section id="pricing" className={styles.pricingSection} aria-labelledby="pricing-heading">
        <div className={styles.sectionHeading}>
          <p className={styles.kicker}>{t("landing.pricing.kicker", "TRANSPARENT & ACCESSIBLE")}</p>
          <h2 id="pricing-heading">
            {t("landing.pricing.headingBefore", "Choose the plan that ")}
            <em>{t("landing.pricing.headingHighlight", "fits your goals.")}</em>
          </h2>
          <p>{t("landing.pricing.lead", "Whether you are learning individually or deploying Lurexa across your entire school or company.")}</p>
        </div>
        ...
        <div className={styles.pricingTabs} role="tablist">
          ...
          >
            {t("landing.pricing.tabInstitutional", "🏢 For Companies, Institutions & Businesses")}
          </button>
        </div>
