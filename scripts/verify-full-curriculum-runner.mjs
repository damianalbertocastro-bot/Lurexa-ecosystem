...
function check(condition, message) { if (!condition) fail(message); else pass(message); }
...
const c2ModuleFile = path.join(repoRoot, "packages/backend/src/curriculum/c2/modules.ts");
...
check(fs.existsSync(c2ModuleFile), "C2 curriculum module source exists");
...
const c2Content = fs.readFileSync(c2ModuleFile, "utf8");
...
check(c2Content.includes("C2_MODULES_1_TO_8"), "C2 defines complete 8-module array");
...
const c2Server = fs.readFileSync(path.join(repoRoot, "packages/backend/src/c2-production-curriculum.server.ts"), "utf8");
...
check(c2Server.includes("buildC2ProductionCurriculum"), "C2 bundle builder is implemented");
...
check(c2Server.includes("english-c2-mastery"), "C2 declares authoritative course ID 'english-c2-mastery'");
...
const capstoneServer = fs.readFileSync(path.join(repoRoot, "packages/backend/src/capstone-multi-level.server.ts"), "utf8");
...
check(placementServer.includes("english-c2-mastery"), "Placement engine targets english-c2-mastery for C2 scores");
...
check(onboardingServer.includes("ensureC2ProductionCurriculumInFirestore"), "Onboarding pipeline provisions C2 curriculum in Firestore");
check(capstoneServer.includes("MultiLevelCapstoneService"), "Multi-level capstone service evaluates summative oral defenses");
...
// Teacher Insights now owns exact-course aggregate operations. Individual learner
// status and Learner Pulse remain in /teacher/students, so the ecosystem runner
// verifies the current product boundary rather than the superseded status-filter UI.
const teacherInsightsPage = fs.readFileSync(path.join(repoRoot, "apps/learn-web/app/teacher/insights/page.tsx"), "utf8");
const teacherStudentsPage = fs.readFileSync(path.join(repoRoot, "apps/learn-web/app/teacher/students/page.tsx"), "utf8");
check(teacherInsightsPage.includes("Course intelligence & enrollment") && teacherInsightsPage.includes("notStarted"), "Teacher insights page is enrollment-aware and exact-course aggregate-first");
check(teacherInsightsPage.includes("/teacher/students") && teacherStudentsPage.includes("Learner Pulse"), "individual learner support remains an explicit Teacher Workspace surface");
check(teacherInsightsPage.includes("Develop yourself in Teach"), "Teacher Workspace provides explicit Learn-to-Teach professional-growth handoff");
...
console.log("\n========================================================");
if (process.exitCode === 1) console.log("  ✗ Full Ecosystem Verification FAILED.");
else console.log("  ✓ All Ecosystem & Curriculum Runner Checks PASSED (100%)");
console.log("========================================================\n");
